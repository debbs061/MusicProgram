	...
	public void FloChartCrawling() {
		try {
			...
			for (int i = 0; i < data.size(); i++) {
				...
				String songName = chartData.get("a.title ellipsis").getAsString(); // 노래 제목
				String updateDateTimeStr = chartData.get("a.title ellipsis").getAsString(); // 노래 발매일
				
				// 가수 정보
				JsonObject singerInfo = (JsonObject) chartData.get("a.artist ellipsis");
				String singerName = singerInfo.get("a.artist ellipsis").getAsString(); // 가수 이름

				// 앨범 정보
				JsonObject albumInfo = (JsonObject) chartData.get("a.albumtitle ellipsis");
				String albumName = albumInfo.get("a.albumtitle ellipsis").getAsString(); // 앨범 이름
